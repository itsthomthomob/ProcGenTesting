...
namespace Broccoli.BroccoEditor
{
	...
	public class SproutLabEditor {
		...
		public class StructureSettings {
			...
			public bool hasCustomTemplates = false;
		}
		#endregion
		...
		SproutCatalog _catalog = null;
		public SproutCatalog catalog {
			get {
				if (_catalog == null) {
					_catalog = SproutCatalog.GetInstance ();
				}
				return _catalog;
			}
		}
		...
		public void LoadBranchDescriptorCollection (BranchDescriptorCollection branchDescriptorCollection, SproutSubfactory sproutSubfactory, string path = "") {
			...
			if (branchDescriptorCollection.descriptorImplId < 0) {
				...
				// Implementation not found.
					...
					viewMode = ViewMode.NotSupported;
				}
			}
		}
		/// <summary>
		...
		public bool LoadCatalogItem (SproutCatalog.CatalogItem itemToLoad) {
			if (itemToLoad != null) {
				string pathToCollection = ExtensionManager.fullExtensionPath + itemToLoad.path;
		...
		public void SetBranchDescriptorCollectionImpl (int descriptorImplId, string descriptorImplName, BranchDescriptorCollection collectionToLoad = null) {
			...
			if (collectionToLoad == null) {
				SproutCatalog.CatalogItem firstItem = catalog.GetFirstItem ();
				LoadCatalogItem (firstItem);
			} else {
				collectionToLoad.descriptorImplId = descriptorImplId;
				collectionToLoad.descriptorImplName = descriptorImplName;
				LoadBranchDescriptorCollection (collectionToLoad, this.sproutSubfactory);
			}
		}
		#endregion
        ...
        public void Draw (Rect windowRect) {
			...
			// TEMPLATES MODE.
				...
				if (currentStructureSettings != null && currentStructureSettings.hasCustomTemplates) {
					currentImplementation.DrawTemplateView (windowRect);
				}
		...
		public void DrawTemplateView (Rect windowRect) {
			...
			// Draw Templates.
			if (_catalog.GetGUIContents ().Count > 0) {
				string categoryKey = "";
				var enumerator = _catalog.contents.GetEnumerator ();
				while (enumerator.MoveNext ()) {
					...
					int height = Mathf.CeilToInt (_catalog.GetGUIContents ()[categoryKey].Count / (float)columns) * catalogItemSize;
					int selectedIndex = 
						GUILayout.SelectionGrid (-1, _catalog.GetGUIContents ()[categoryKey].ToArray (), 
					...
					if (selectedIndex >= 0 &&
					   EditorUtility.DisplayDialog (MSG_LOAD_CATALOG_ITEM_TITLE, 
						...
						SproutCatalog.CatalogItem itemToLoad = _catalog.GetItemAtIndex (categoryKey, selectedIndex);
